...
class TwoPlayerOneHead(SnakeState):

    def __init__(self, field: Field):
        super(TwoPlayerOneHead, self).__init__(field, 100)
        half = field.size // 2
        self.snake1 = OneHeadedSnake(field, Vec(half - 2, half - 1), UP, SNAKE_BODY_COLORS[0], SNAKE_HEAD_COLORS[0])
        ...
        self.snake2 = OneHeadedSnake(field, Vec(half + 1, half), DOWN, SNAKE_BODY_COLORS[1], SNAKE_HEAD_COLORS[1])
    ...
    def input(self, event: pygame_event):
        ...
        super(TwoPlayerOneHead, self).input(event)
    ...
    def draw(self, screen: Surface):
        super(TwoPlayerOneHead, self).draw(screen)
